...
// +----+-----------------+
// | 32 | context pid     | pid of the current context (unique identifier for the process)
// | 33 | parent context  | pointer to the parent context
// | 34 | is parent       | flag indicating whether the current context is a parent process
// | 35 | child exit pid  | pid of the child that has exited
...
void copy_virtual_memory(uint64_t *child, uint64_t *parent, uint64_t vaddr, uint64_t end)
{
  while (vaddr <= end) {
    if (is_virtual_address_mapped(get_pt(parent), vaddr)) {
      map_and_store(child, vaddr, load_virtual_memory(get_pt(parent), vaddr));
    }
...
void implement_fork(uint64_t *context)
{
  ...
  set_highest_hi_page(child_context, get_highest_hi_page(context));
  set_highest_lo_page(child_context, get_highest_lo_page(context));

  set_code_seg_size(child_context, get_code_seg_size(context));
  set_code_seg_start(child_context, get_code_seg_start(context));
  set_data_seg_start(child_context, get_data_seg_start(context));
  set_data_seg_size(child_context, get_data_seg_size(context));  
  set_heap_seg_start(child_context, get_heap_seg_start(context));

  set_program_break(child_context, get_program_break(context));
  
  set_pc(child_context, get_pc(context));
  set_name(child_context, get_name(context));

  // copy: text data heap
  copy_virtual_memory(child_context, context, get_code_seg_start(context), get_program_break(context));
  
  // copy: stack
  copy_virtual_memory(child_context, context, *(get_regs(context) + REG_SP), HIGHESTVIRTUALADDRESS);

  // copy: registers
  itr = 0;
  while (itr < NUMBEROFREGISTERS)
  {
    *(get_regs(child_context) + itr) = *(get_regs(context) + itr);
    itr = itr + 1;
  }
...
void emit_wait()
{
  ...
  emit_load(REG_A0, REG_SP, 0); // wstatus
  emit_addi(REG_SP, REG_SP, WORDSIZE);
...
uint64_t *delete_context(uint64_t *context, uint64_t *from)
{
  from = remove_context(context, from);
  ...
  while (1)
  {
    ...
    else
    {
      ...
      else 
      {
        ...
        if (search_context(to_context, used_contexts) == MY_CONTEXT) {
...
uint64_t selfie_run_mipsterOS(uint64_t machine)
{
  ...
  number_of_processes = atoi(peek_argument(0));
  ...
  init_memory(number_of_processes);
